...
Wires together the five stages from the paper:

  1. Binning           (done by the data loader)
  ...
  5. Final prediction (train final XGBoost; AUC on test set)

Returns a `PipelineResult` containing everything you need for analysis and
plotting (patterns, hyperparams, AUCs, the trained booster, etc).
"""
...
def run_pipeline(ds: Dataset,
    ...
    """Run all five stages and return everything."""
    t = {}
    ...
    # --- Stage 5: final training and prediction ---
    t0 = time()
    if verbose: print("\n=== Stage 5: final training and test evaluation ===")
    # Paper trains the final model on the training set only.
    ...
    # SHAP on test set for downstream interpretation
