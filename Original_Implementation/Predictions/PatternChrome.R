...
for (cell_line in cell_lines) {
  ...
  save(patterns, file = paste(cell_line, date, "patterns.RData", sep = "_")) # Pattern characteristics for the particular cell line are stored
  #train_df <- original_train_df
  save(train_df, file = paste(cell_line, date, "train_df.RData", sep = "_")) # Training set consisting of the feature matrix with all pattern frequencies before backward elimination

  # save validation set and test set seperatly
  save(validation_test_df, file = paste(cell_line, date, "validation_test_df.RData", sep = "_")) # Validation and test set consisting of the feature matrix with all pattern frequencies before backward elimination
  save(validation_df, file = paste(cell_line, date, "validation_df.RData", sep = "_"))
  save(test_df, file = paste(cell_line, date, "test_df.RData", sep = "_"))
  
  #### save stats ####
  New_line_str <- paste(grep(cell_line,cell_lines),cell_line, test_accuracy, Sys.time()-t1, ncol(train_df)-1,sep=",")
  write(New_line_str, file = paste0(Paths$dataset_path,"/stats.csv"), append = TRUE, sep = "\n") 
  
  ### save Model ####
  saveRDS(XGB_model, file = paste0(Paths$dataset_path,"/",cell_line,"/","XGB_model",date,".rds"))
  saveRDS(res, file = paste0(Paths$dataset_path,"/",cell_line,"/","Parameters",date,".rds"))
