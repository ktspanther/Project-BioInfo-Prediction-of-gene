...
# Preparation: Before running the code check the Path_config.yaml file and make
# sure that you adjusted its content to your architectur. Make sure that you have
# all the input data in the correct directory (ChIP-Seq in
# Binned_sequencing_data - main_path parameter in Path_config.yaml,
# RNA_seq in PatternChrome directory - RNAseq_path parameter in Path_config.yaml)
# and that you have created a Datasets directory to store the data if desired.
...
rounding_decimal <- 5 # roundig decimal for AUC score
...
max_distance <- (floor(10000/bin_size/3))^2 # maximum length of search area for patterns
...
Objective <- function(pars){
  ...
  round(auc(response = optim_df[,1], 
    ...
    nrounds = nrounds, eta = eta, verbose = 0, objective = "binary:logistic", nthread = Paths$n_workers,
...
hyperparameter_tuning <- function(hyperparams,Dataset,fn_train){# xgb_validation substituted by Dataset
  round(auc(response = getinfo(Dataset, name = "label"), 
    ...
    objective = "binary:logistic", eval.metric = "auc",tree_method = "hist"), Dataset), quiet = T),rounding_decimal)
}

#### Sample splitting function ####
# Function that splits the Set in N_splits+1 Groups while preseving the ratio of labels
...
stats <- c()
for (cell_line in cell_lines[5:length(cell_lines)]) {
  print(cell_line)
  t1 <- Sys.time()
  #### Load CHIP_Seq data ####
  ...
  RNA <- get_alternative_cutoff(RNA)
  names(RNA) <- rownames(H3K27me3)
  
  #### Train, validation and test subsets ####
  set.seed(42)
  train_genes <- names(RNA)[sample.split(RNA, SplitRatio = 6600)] # 6600 instead of 6601 since 6601 is odd and therefore does not allow to have balanced labels
  ...
  while(improving){
    ...
    res <- psoptim(par = rep(NA, length(objective_upper)), fn = Objective, 
      ...
      control = list(fnscale = -1,vectorize = T, abstol = -1, 
        ...
        trace = report, REPORT = report_frequency,maxit.stagnate = maxit_stagnate))
  ...
  stopCluster(cl)
  print(paste("Training finished with ", -res$value, " accuracy. Number of trained patterns: ", ncol(train_df)-1, ". Time: ", Sys.time()-t1))
  ...
  validation_df <- validation_test_df[!split,]
    ...
    validation_accuracy <- round(auc(response = validation_df[,1], 
      ...
      nrounds = nrounds, eta = eta, nthread = Paths$n_workers, objective = "binary:logistic", verbose = 0, 
      eval.metric = "auc", tree_method = "hist"), 
    ...
    while(pattern != 2){
      ...
      be_accuracy <- round(auc(response = be_validation_df[,1], 
        ...
        nthread = Paths$n_workers, nrounds = nrounds, eta = eta, objective = "binary:logistic", verbose = 0, 
        eval.metric = "auc"), 
  ...
  print(paste("Run completed for cell line ", cell_line, ". Duration: ", Sys.time()-t1, " minutes/hours. Test AUC score: ", test_accuracy, ". Number of patterns learned: ", ncol(train_df)-1))
  ...
  save(patterns, file = paste(cell_line, date, "patterns_alt_co.RData", sep = "_")) # Pattern characteristics for the particular cell line are stored
  #train_df <- original_train_df
  save(train_df, file = paste(cell_line, date, "train_alt_co_df.RData", sep = "_")) # Training set consisting of the feature matrix with all pattern frequencies before backward elimination

  # save validation set and test set seperatly
  save(validation_test_df, file = paste(cell_line, date, "validation_test_alt_co_df.RData", sep = "_")) # Validation and test set consisting of the feature matrix with all pattern frequencies before backward elimination
  save(validation_df, file = paste(cell_line, date, "validation_alt_co_df.RData", sep = "_"))
  save(test_df, file = paste(cell_line, date, "test_alt_co_df.RData", sep = "_"))
  
  #### save stats ####
  New_line_str <- paste(grep(cell_line,cell_lines),cell_line, test_accuracy, Sys.time()-t1, ncol(train_df)-1,sep=",")
  write(New_line_str, file = paste0(Paths$analysis_data,"stats_alternative_cutoff.csv"), append = TRUE, sep = "\n") 
  
  ### save Model ####
  saveRDS(XGB_model, file = paste0(Paths$dataset_path,"/",cell_line,"/","XGB_model_alt_co",date,".rds"))
  saveRDS(res, file = paste0(Paths$dataset_path,"/",cell_line,"/","Parameters_alt_co",date,".rds"))
  
  rm(list = c("XGB_model","res","validation_df","test_df","validation_test_df","train_df","patterns"))
  gc()
