...
2. **PSO topology.** The paper uses R's `psoptim()` (a global-best PSO). Our Python PSO uses the same hyperparameters (`w` decreasing 0.8→0.4, `c.p = c.g = 2.05`) but without the parallel cluster evaluation the R code uses.
3. **XGBoost explainer.** The paper uses the R `xgboostExplainer` package. We use XGBoost's native `pred_contribs=True` (TreeSHAP). The values are not identical but capture the same per-feature contributions.
4. **Stopping budgets.** Default `n_patterns_max=20` keeps demo runs short. Increase via CLI for paper-comparable results.
5. **The synthetic test mode.** Not in the paper — added so you can run the pipeline without downloading any data.
